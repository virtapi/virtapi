...
CREATE UNIQUE INDEX `name_UNIQUE` ON `virtapi`.`cache_option` (`name` ASC)  COMMENT '';
...
CREATE UNIQUE INDEX `name_UNIQUE` ON `virtapi`.`storage_type` (`name` ASC)  COMMENT '';
...
CREATE TABLE IF NOT EXISTS `virtapi`.`storage` (
  ...
  `write_bps_limit` INT NULL COMMENT '',
  `read_bps_limit` INT NULL COMMENT '',
